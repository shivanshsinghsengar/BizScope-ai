...
function ScoreBar({ label, score, reason }) {
  const color = score <= 4 ? '#ef4444' : score <= 7 ? '#f59e0b' : '#3b82f6';
  return (
...
function SectionContent({ title, content }) {
  ...
  if (isVerdict) {
    ...
    return (
      <div>
        <div style={{ fontSize: '15px', fontWeight: '700', color: 'var(--text)', marginBottom: '20px', padding: '14px', background: 'var(--surface2)', borderRadius: '12px', borderLeft: '3px solid #3b82f6' }}>
          {verdict}
        </div>
    ...
    if (risks.length > 0) {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {risks.map((r, i) => {
            const probColor = r.prob === 'Low' ? '#3b82f6' : r.prob === 'High' ? '#ef4444' : '#f59e0b';
            return (
              <div key={i} style={{ background: 'var(--surface2)', borderRadius: '12px', padding: '14px', borderLeft: `3px solid ${probColor}` }}>
                ...
                {r.kill && <div style={{ fontSize: '12px', color: '#3b82f6' }}>🛡️ {r.kill}</div>}
              </div>
            );
          })}
        </div>
  ...
  if (isExpansion) {
    ...
    if (angles.length > 0) {
      const revColors = { Low: '#64748b', Medium: '#f59e0b', High: '#3b82f6', Moonshot: '#a78bfa' };
      return (
    ...
    <div style={{ fontSize: '13px', color: 'var(--text2)', lineHeight: '1.9', whiteSpace: 'pre-line' }}>
      {content.split('\n').map((line, i) => {
        if (line.startsWith('### ') || line.startsWith('PHASE ') || line.startsWith('BOOTSTRAP') || line.startsWith('FUNDED')) {
          return <div key={i} style={{ fontWeight: '700', color: '#3b82f6', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.08em', marginTop: '16px', marginBottom: '6px' }}>{line.replace(/^###\s*/, '')}</div>;
        }
...
function CollapsibleCard({ section, index }) {
  ...
  const icons = { '🎯': '#3b82f6', '🌍': '#60a5fa', '⚔️': '#ef4444', '🚀': '#3b82f6', '💰': '#f59e0b', '🆕': '#a78bfa', '⚠️': '#f87171', '📅': '#06b6d4', '💬': '#3b82f6' };
  const sectionImages = {
  ...
  const accentColor = icons[emoji] || '#3b82f6';
