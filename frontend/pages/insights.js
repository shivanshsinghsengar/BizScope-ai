    ...
    <Layout>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '32px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <h1 style={{ fontSize: '32px', fontWeight: '800', color: 'white', marginBottom: '8px' }}>🤖 AI Insights</h1>
          <p style={{ color: '#475569', fontSize: '15px' }}>Market intelligence and business recommendations</p>
        </div>

        {/* AI Suggestions */}
        <div style={{ background: '#0f172a', border: '1px solid #4f46e530', borderRadius: '24px', padding: '32px', marginBottom: '24px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '24px' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '18px', fontWeight: '700', color: 'white' }}>AI Business Recommendations</div>
              <div style={{ fontSize: '13px', color: '#475569' }}>Powered by Gemini / GPT market analysis</div>
            </div>
            {polling && (
          ...
          {polling && !aiText ? (
            <div style={{ textAlign: 'center', padding: '40px' }}>
              ...
              <div style={{ color: '#475569', fontSize: '14px' }}>AI is analyzing the market data, please wait...</div>
            </div>
          ) : noAI ? (
            <div>
              <div style={{ background: '#1e293b', borderRadius: '14px', padding: '14px 18px', marginBottom: '12px', fontSize: '13px', color: '#64748b' }}>
                ℹ️ AI key not configured — showing data-driven recommendations instead.
              </div>
              {[...data.categoryStats].reverse().slice(0, 5).map((s, i) => (
                <div key={i} style={{ background: '#1e293b', borderRadius: '14px', padding: '18px 20px', marginBottom: '10px', borderLeft: `3px solid ${categoryColors[s.category] || '#6366f1'}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <span style={{ fontWeight: '700', color: 'white', fontSize: '15px' }}>{categoryIcons[s.category] || '🏪'} {s.category}</span>
            ...
            <div style={{ color: '#cbd5e1', lineHeight: '1.9', fontSize: '15px', whiteSpace: 'pre-line' }}>{aiText}</div>
          )}
        </div>

        {/* Risk formula explanation */}
        <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '20px', padding: '24px', marginBottom: '24px' }}>
          <div style={{ fontSize: '15px', fontWeight: '700', color: 'white', marginBottom: '16px' }}>📐 How Risk is Calculated</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', marginBottom: '16px' }}>
            {[
              ...
              <div key={f.label} style={{ background: '#1e293b', borderRadius: '14px', padding: '16px' }}>
                <div style={{ fontSize: '22px', marginBottom: '8px' }}>{f.icon}</div>
                <div style={{ fontWeight: '600', color: 'white', fontSize: '13px', marginBottom: '4px' }}>{f.label}</div>
                <div style={{ fontSize: '20px', fontWeight: '800', color: '#6366f1', marginBottom: '4px' }}>{f.weight}</div>
                <div style={{ fontSize: '11px', color: '#475569' }}>{f.desc}</div>
              </div>
            ))}
          </div>
          <div style={{ background: '#1e293b', borderRadius: '12px', padding: '14px 18px', fontSize: '13px', color: '#64748b', fontFamily: 'monospace' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
          {/* Best opportunities */}
          <div style={{ background: '#0f172a', border: '1px solid #10b98125', borderRadius: '20px', padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
              ...
              <div>
                <div style={{ fontWeight: '700', color: 'white', fontSize: '15px' }}>Best Opportunities</div>
                <div style={{ fontSize: '12px', color: '#475569' }}>Lowest competition categories</div>
              </div>
            </div>
            {best.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', background: '#1e293b', borderRadius: '14px', marginBottom: '10px', border: '1px solid #10b98115' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: '600', color: 'white', fontSize: '14px' }}>{s.category}</div>
                  <div style={{ fontSize: '11px', color: '#475569' }}>Only {s.count} competitors nearby</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: '11px', color: '#475569', marginTop: '4px' }}>Score: {s.competitorScore?.toFixed(1)}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Saturated markets */}
          <div style={{ background: '#0f172a', border: '1px solid #ef444425', borderRadius: '20px', padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
              ...
              <div>
                <div style={{ fontWeight: '700', color: 'white', fontSize: '15px' }}>Saturated Markets</div>
                <div style={{ fontSize: '12px', color: '#475569' }}>High competition — proceed carefully</div>
              </div>
            </div>
            {worst.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', background: '#1e293b', borderRadius: '14px', marginBottom: '10px', border: '1px solid #ef444415' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: `${categoryColors[s.category] || '#6366f1'}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px' }}>{categoryIcons[s.category] || '🏪'}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: '600', color: 'white', fontSize: '14px' }}>{s.category}</div>
                  <div style={{ fontSize: '11px', color: '#475569' }}>{s.count} competitors • ⭐ {s.avgRating}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px' }}>
          {data.categoryStats?.map((s, i) => (
            <div key={i} style={{ background: '#0f172a', border: `1px solid ${(categoryColors[s.category] || '#6366f1')}20`, borderRadius: '16px', padding: '20px', textAlign: 'center', transition: 'transform 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-4px)'}
              onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>
              <div style={{ fontSize: '28px', marginBottom: '10px' }}>{categoryIcons[s.category] || '🏪'}</div>
              <div style={{ fontWeight: '700', color: 'white', fontSize: '14px', marginBottom: '6px' }}>{s.category}</div>
              <div style={{ fontSize: '28px', fontWeight: '800', color: categoryColors[s.category] || '#6366f1', marginBottom: '4px' }}>{s.count}</div>
              <div style={{ fontSize: '11px', color: '#475569' }}>competitors</div>
              ...
              <div style={{ fontSize: '10px', color: '#334155', marginTop: '4px' }}>Risk Score: {s.riskScore ?? '—'}/100</div>
            </div>
          ))}
        </div>
      </div>
